...
<body>
  ...
  <canvas id="bigDashboardChart"></canvas>
	  ...
	  <ul>
		<li>
		<a href="admin
    reserv.php?validAnn=false"><h4><b>Annuler une réservation</b></h4></a>
		</li>
		</ul>
		<?php
		...
		if($_GET['validAnn']=="false")
			{
				$requete1 = $bdd->prepare('SELECT * from annonce WHERE waitReserv=:waitReserv and reserved=:reserved');
				$requete1->execute(array('waitReserv'=> 1,
										 'reserved'=> 1
										));
	...
	{
	$update = $bdd->prepare('UPDATE annonce set reserved = :reserved, dateCancelReserv=NOW() WHERE id = :id');
	$update->execute(array('reserved'=> 0,
						   'id'=> $_GET['abort']
					));
	...
	?>
				...
				<div class="col-md-15 col-sm-15 col-xs-15">
									<div class="categorie-item">
										...
										<div>
										<a href="adminreserv.php?validAnn=false&abort=<?php echo $result['id']?>">
										<span class="btn btn-dark ccm_sticker__btn" aria-hidden="true">
										<i class="glyphicon glyphicon-download-alt"></i>
										Annuler réservation</span>
										</a>
										<a href="admin
                    reserv.php?validAnn=false&detailsville=<?php echo $result['id']?>">
										<span class="btn btn-dark ccm_sticker__btn" aria-hidden="true">
										<i class="glyphicon glyphicon-download-alt"></i>
